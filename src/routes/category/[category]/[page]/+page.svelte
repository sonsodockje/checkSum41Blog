...
<svelte:head>
	<title>{data.category} - Page {data.currentPage}</title>
</svelte:head>

<div class="m-auto w-xl">
	<h1 class="mb-10 text-center text-3xl font-extrabold uppercase text-white md:text-4xl">{data.category}</h1>
	...
	<div class="mt-12 flex items-center justify-between text-sm md:text-base">
		{#if data.currentPage > 1}
			<a
				href="{base}/category/{data.category}/{data.currentPage - 1}"
